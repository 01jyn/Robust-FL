import sys
sys.path.append(r'/home/yanan/Robust_FL-Master-cf/Robust_FL-master')
print(sys.path)
#sys.path.append("..")
...
def update_model_via_private_data_old(network,private_epoch,private_dataloader,loss_function,optimizer_method,learing_rate,logger, regularizer_loss):
    if loss_function =='CE':
    ...
    for epoch_index in range(CommunicationEpoch):
        ...
        for participant_index in range(N_Participants):
            ...
            if sys.argv[2] != None and sys.argv[3] != None:
                type_of_reg = sys.argv[2]
                lambda_reg = float(sys.argv[3])

                if type_of_reg == 'l1':
                    reg_loss = L1Regularizer(model=network, lambda_reg=0.01)
                    reg_loss.lambda_reg = lambda_reg
                if type_of_reg == 'l2':
                    reg_loss = L2Regularizer(model=network, lambda_reg=0.01)
                    reg_loss.lambda_reg = lambda_reg
                if type_of_reg == 'EL':
                    reg_loss = ElasticNetRegularizer(model=network, lambda_reg=0.01)
                    reg_loss.lambda_reg = lambda_reg
                    reg_loss.alpha_reg = 0.9  #这儿写死了
                # if type_of_reg == 'GL':
                #     reg_loss = GroupLassoRegularizer(model=network, lambda_reg=0.01)
                #     reg_loss.lambda_reg = lambda_reg
                # if type_of_reg == 'SGL':
                #     reg_loss = GroupSparseLassoRegularizer(model=network, lambda_reg=0.01)
                #     reg_loss.lambda_reg = lambda_reg        
...
#------------------------正则化初始化结束----------------------------- 
        ...
        for participant_index in range(N_Participants):
            ...
            logger=logger,regularizer_loss = reg_loss)
            else:
